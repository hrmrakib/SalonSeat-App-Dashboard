      ...
      <div className='container mx-auto space-y-8'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6'>
          <div className='bg-white p-6 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-4'>
            <div className='w-12 h-12 rounded-xl bg-[#eefafb] flex items-center justify-center text-[#159a9c]'>
          ...
          <div className='bg-white p-6 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-4'>
            ...
            <div>
              <p className='text-xs font-medium text-slate-400'>Salon Owners</p>
              <h3 className='text-2xl font-bold text-slate-800'>
        ...
        {isLoading ? (
          ...
          <div className='text-center py-20 text-slate-400 bg-white rounded-2xl border border-slate-100 shadow-xs text-sm'>
            ...
            <table className='w-full text-sm'>
              <thead>
                <tr className='bg-[#159a9c] text-white'>
                  <th className='text-left px-6 py-4 font-semibold text-xs tracking-wide'>
                    Listing Name
                  </th>
                  <th className='text-left px-6 py-4 font-semibold text-xs tracking-wide'>
                    Owner
                  ...
                  <th className='text-center px-6 py-4 font-semibold text-xs tracking-wide'>
                    Action
                  </th>
                </tr>
              </thead>
                ...
                {listings.map((listing: Listing) => (
                  ...
                  >
                    ...
                    <td className='px-6 py-4'>
                      <span className='inline-block bg-[#eefafb] text-[#159a9c] border border-[#d2f1f3] text-[10px] font-bold px-2.5 py-1 rounded-md tracking-wider uppercase'>
                        {listing.rental_type}
                      </span>
                    </td>
                    <td className='px-6 py-4 text-xs font-bold text-[#159a9c]'>
                    ...
                    <td className='px-6 py-4 text-center'>
                      <button
                        ...
                        className='w-7 h-7 rounded-full border border-slate-300 text-slate-400 hover:border-[#159a9c] hover:text-[#159a9c] flex items-center justify-center mx-auto transition-colors text-xs font-bold'
                        title='View Details'
                      >
                        ⓘ
                      </button>
        ...
        {!isLoading && listings.length > 0 && meta.total_pages > 1 && (
          <div className='flex items-center justify-between border border-slate-100 bg-white px-5 py-4 rounded-2xl shadow-xs'>
            <div className='flex flex-1 justify-between sm:hidden'>
              <button
                onClick={() => setPage((prev) => Math.max(prev - 1, 1))}
                disabled={page === 1}
                className='inline-flex items-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors'
              >
                Previous
              </button>
              <button
                ...
                className='ml-3 inline-flex items-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors'
              >
                Next
              </button>
            </div>
              ...
              <div>
                ...
                >
                  <button
                    onClick={() => setPage((prev) => Math.max(prev - 1, 1))}
                    disabled={page === 1}
                    className='px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-medium text-slate-500 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors'
                  >
                    ← Prev
                  </button>
        ...
        <div className='fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4'>
          <div className='bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl relative flex flex-col max-h-[90vh]'>
            ...
            <div className='p-6 overflow-y-auto space-y-5'>
              ...
              <div className='space-y-3 border-t border-b border-slate-100 py-4 text-xs font-medium'>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span className='text-[10px] bg-[#eefafb] text-[#159a9c] border border-[#d2f1f3] px-2.5 py-0.5 rounded-md font-bold uppercase'>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <div className='flex items-center gap-1.5'>
                    ...
                    <span className='text-[9px] bg-[#eefafb] text-[#159a9c] font-bold px-1.5 py-0.5 rounded border border-[#d2f1f3] tracking-wide uppercase'>
                      Verified
                    </span>
                  </div>
                </div>
            ...
            <button
              ...
              className='absolute top-3 right-3 bg-slate-900/40 hover:bg-slate-900/60 text-white w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold transition-all'
            >
              ✕
            </button>
          </div>
